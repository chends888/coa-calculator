...
import GoldKey from "./tabs/GoldKey";

import useSkillData from "./hooks/useSkillData";

// Utility tabs live in their own list, separate from the skill tabs, so
// adding a new utility later (another chest type, etc.) is just one more
// entry here - no need to touch the skill tab wiring at all.
const UTILITY_TABS = [{ key: "goldkey", label: "Gold Key", component: GoldKey }];
...
const Home = (props) => {
  ...
  // Top-level category: "skills" shows the existing skill tabs exactly as
  // before; "utilities" shows a separate tab bar driven by UTILITY_TABS.
  const [category, setCategory] = useState("skills");
  const [selectedUtilityTab, setSelectedUtilityTab] = useState(0);
    ...
    <>
      ...
      {category === "utilities" && (
        <>
          <Box sx={{ width: "100%" }}>
            <Box sx={{ borderBottom: 1, borderColor: "divider", marginBottom: 1 }}>
              <Tabs
                ...
                onChange={(event, newValue) => setSelectedUtilityTab(newValue)}
                variant="scrollable"
              >
          ...
          {UTILITY_TABS.map((tab, index) => {
            const UtilityComponent = tab.component;
            return (
              <Box key={tab.key} sx={{ display: selectedUtilityTab === index ? "block" : "none" }}>
                <UtilityComponent />
              </Box>
            );
          })}
